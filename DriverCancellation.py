...
def updaterec():
        ...
        res=mycur.fetchone()
        if res:
            sql="update driverbooking set  Bookstatus='" + t9.get() + "' where bookno='" + drid + "'"
            mycur.execute(sql)
            mydb.commit()
          
            l18.configure(text= "Booking Cancelled")
            
        else:
            l18.configure(text= "Booking Number Missing")
def viewrec():
        drid=t1.get()
        sql="select * from driverbooking where bookno='" + drid +"'"
        mycur.execute(sql)
...
def reportrec():
        ...
        if res:
            ...
            print()
            print("------------------------------------------------------------------------------------------------------------------------------")
            print("                                 Booking Cancellation Details  Report")
            print("------------------------------------------------------------------------------------------------------------------------------")
            ...
            sqlst=pd.read_sql("select * from driverbooking where Bookstatus='Cancelled'",mydb)
...
l2 = Label(win,text=" Booking Cancellation Details Maintenance")
...
b1 = Button(win,text=" View ", command=viewrec)
b1.grid(column=3,row=6,ipadx=20)

b2 = Button(win,text=" Update ", command=updaterec)
